...
# Wait until the PostgreSQL pod is running and obtain its IP address
POD_NAME=""
POSTGRES_SERVER=""
while [ -z "${POSTGRES_SERVER}" ]; do
  # Get the name of the PostgreSQL pod from the deployment
  if [ -z "${POD_NAME}" ]; then
  ...
  if [ -n "${POD_NAME}" ]; then
    POSTGRES_SERVER=$(kubectl get pod ${POD_NAME} -o jsonpath='{.status.podIP}')
  fi
